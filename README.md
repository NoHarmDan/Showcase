...
Traditionally, the UI would be implemented in its own module, but due to the scale of this example, it was merged within the `app` module, which typically serves the top-level module which puts all the other modules together, but would be nearly empty here otherwise. This module also directs the dependency injection using [Koin](https://insert-koin.io/).
...
+ Compose views at the top level
